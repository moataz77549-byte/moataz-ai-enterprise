...
export interface ButtonProps
  ...
  asChild?: boolean;
  /** Shows a spinner and disables interaction. Defaults to false; fully opt-in. */
  loading?: boolean;
}
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, loading = false, disabled, children, ...props }, ref) => {
    ...
    // Slot (asChild) requires exactly one child element, so the loading spinner
    // is only composed in for plain <button> rendering.
    if (asChild) {
      return (
        <Comp
